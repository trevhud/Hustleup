...
// @downloadURL https://userscripts.org/scripts/source/160501.user.js
// @updateURL   https://userscripts.org/scripts/source/160501.user.js
// ==/UserScript==
...
{
  var csvData = 'requesterId,requesterName,pending,reward\n';
  for (var i=0; i<results.length; i++) {
    csvData += HITStorage.format_pending_line_csv(results[i]);
  }
...
{
  var csvData = 'requesterId,requesterName,hits,reward,rejected,pending\n';
  for (var i=0; i<results.length; i++) {
    csvData += HITStorage.format_overview_line_csv(results[i]);
  }
  location.href='data:text/csv;charset=utf8,' + encodeURIComponent(csvData);
}
...
{
  var csvData = 'Date,Submitted,Approved,Rejected,Pending,Earnings\n';
  for (var i=results.length-1; i>=0; i--) {
    csvData += HITStorage.format_status_line_csv(results[i]);
  }
  location.href='data:text/csv;charset=utf8,' + encodeURIComponent(csvData);
}
...
{
  var csvData = 'hitId,date,requesterName,requesterId,title,reward,status,feedback\n';
  for (var i=0; i<results.length; i++) {
    csvData += HITStorage.format_csv_line(results[i]);
  }
  location.href='data:text/csv;charset=utf8,' + encodeURIComponent(csvData);
}
...
HITStorage.format_csv_line = function(hit)
{
  ...
  line += '"' + hit.hitId.trim() + '",';
  line += '"' + hit.date.trim() + '",';
  line += '"' + hit.requesterName.trim() + '",';
  line += '"' + hit.requesterId.trim() + '",';
  line += '"' + hit.title.trim() + '",';
  line += hit.reward.toFixed(2) + ',';
  line += '"' + hit.status.trim().replace(/\&nbsp;/g,' ') + '",';
  line += '"' + hit.feedback.trim() + '"';
  line += '\n';
  return line;
}
